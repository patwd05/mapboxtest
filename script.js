...
function filterSchoolsByIsochrone(isochroneData) {
    ...
    mpsStudentData.features.forEach(student => {
        ...
        if (turf.booleanPointInPolygon(studentPoint, isochroneData.features[0])) {
            const schoolName = Number(student.properties['USER_ATTEN']) || 0;
            if (schoolName) {
    ...
    schoolData.features = schoolData.features.filter(school => schoolCounts[school.properties['Program']?.toLowerCase()]);
    schoolData.features.forEach(school => {
        school.properties.studentCount = schoolCounts[String(school.properties['Program'])] || 0;
    });
...
// ✅ Function to Handle Isochrone Distance Selection
document.getElementById("isochrone-800").addEventListener("click", () => {
    isochroneDistance = 800;
});
document.getElementById("isochrone-1600").addEventListener("click", () => {
    isochroneDistance = 1600;
});
document.getElementById("isochrone-3200").addEventListener("click", () => {
    isochroneDistance = 3200;
});

// ✅ Function to Fetch and Display Isochrone
function addIsochrone(lng, lat) {
    if (!isochroneEnabled) return;
    const isoUrl = `https://api.mapbox.com/isochrone/v1/mapbox/walking/${lng},${lat}?contours_meters=${isochroneDistance}&polygons=true&access_token=${mapboxgl.accessToken}`;
